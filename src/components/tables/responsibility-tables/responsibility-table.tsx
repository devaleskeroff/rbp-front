...
const ResponsibilityTable: React.FC<ResponsibilityTablePropsT> = ({ setWithHistory, sort }) => {
    // STORES
    ...
    useEffect(() => {
        ...
        if (directoryId !== 0) {
            ResponsibilityService.GetResponsibilityDocuments(directoryId, (err, res) => {
                if (err || !res) {
                    return setResponsibilityError(true)
                }
                setResponsibilityDocuments(sortDocuments([res.data.files, res.data.directories]))
    ...
    const handleDirectoryDelete = (folderId: number) => {
        ResponsibilityService.DeleteDirectory(folderId, (err, _res) => {
            ...
            if (directoryId === 0) {
                setResponsibilityCommonDocs([commonDocuments[0], commonDocuments[1].filter(dir => dir.id !== folderId)])
            } else {
                setResponsibilityDocuments(sortDocuments([currentDocuments[0], currentDocuments[1].filter(dir => dir.id !== folderId)]))
            }
            close()
        })
    }
    ...
    const handleFileDelete = (id: number) => {
        return ResponsibilityService.DeleteFile(id, (err, _res) => {
            ...
            if (directoryId === 0) {
                setResponsibilityCommonDocs([commonDocuments[0].filter(file => file.id !== id), commonDocuments[1]])
            } else {
                setResponsibilityDocuments(sortDocuments([currentDocuments[0].filter(file => file.id !== id), currentDocuments[1]]))
            }
            close()
        })
    }
