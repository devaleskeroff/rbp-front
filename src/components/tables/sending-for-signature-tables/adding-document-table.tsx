...
export type AddingDocumentTablePropsT = {
    ...
    files: UnitFileForAddingToPositionDataT[] | null
    signMode?: boolean,
    archiveHandler: (fileId: number) => void
}

const AddingDocumentTable: React.FC<AddingDocumentTablePropsT> = props => {
    const { selectedDocs, onSelect, files, signMode, archiveHandler } = props
            ...
            <tr key={idx}>
                <td>
                    <label htmlFor={`${id}`} className={ clsx(tableStyle.column_fixed_height) }>
                        <input type="checkbox" className={ clsx(tableStyle.checkbox_item) } hidden name=""
                               ...
                               checked={!!selectedDocs.find(selectedDocId => selectedDocId === id)}
                        />
                        <label htmlFor={`${id}`}>
                ...
                <td>
                    <div className={ clsx(style.actions_col) }>
                        ...
                        <img src={ ArchiveIcon } alt="" onClick={() => sendFileToArchive(file.id)} />
                    </div>
                </td>
            </tr>
        )
