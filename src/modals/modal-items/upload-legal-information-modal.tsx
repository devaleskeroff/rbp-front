...
// STORE
import {
    $CommonLegalInformationDocuments, $LegalInformationDocuments,
    pushToLegalInformationDocs, setLegalInformationCommonDocs,
    setLegalInformationDocuments
} from '@store/legal-information-store'
// STYLES
import style from '@scss/pages/responsibility.module.scss'

const UploadLegalInformationModal = () => {
    // STORES
        ...
        if (modalData.editMode) {
            ...
            return LegalInformationService.UpdateFile(modalData.file.id, title.value, (err, _res) => {
                ...
                if (+(Querystring.folder_id as string) === 0) {
                    setLegalInformationCommonDocs([commonDocuments[0].map(file => {
                ...
                setLegalInformationDocuments([currentDocuments[0].map(file => {
                    if (file.id === modalData.file.id) {
                        file.title = title.value
                    }
                    return file
        ...
        LegalInformationService.UploadFiles(formData, +(Querystring.folder_id as string) || 0, (err, res) => {
            ...
            if (+(Querystring.folder_id as string) === 0) {
                res.data.forEach((file: any) => {
                    pushToLegalInformationDocs([file, null])
                })
                return close()
            }
            res.data.forEach((file: any) => {
...
export default UploadLegalInformationModal